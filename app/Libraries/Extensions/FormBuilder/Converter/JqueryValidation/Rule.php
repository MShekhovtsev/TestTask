...
class Rule extends \App\Libraries\Extensions\FormBuilder\Converter\Base\Rule {
	...
	{
		return ['accept' => "image/*", 'data-rule-image' => 'true'];
	}
	...
	public function min($parsedRule, $attribute, $type) 
	{
		switch ($type) 
		{
			...
			case 'file':
				return ['data-rule-minfilesize' => vsprintf("%1s", $parsedRule['parameters'])];
				break;
	...
	public function max($parsedRule, $attribute, $type) 
	{
		switch ($type) 
		{
			...
			case 'file':
				return ['data-rule-maxfilesize' => vsprintf("%1s", $parsedRule['parameters'])];
				break;
	...
	public function between($parsedRule, $attribute, $type) 
	{
		switch ($type) 
		{
			...
			case 'file':
				return ['data-rule-filesizerange' => vsprintf("%1s,%2s", $parsedRule['parameters'])];
				break;
	...
	public function mimes($parsedRule, $attribute, $type)
	{
		...
		return ['data-rule-accept' => $rule];
	}

    public function confirmed($parsedRule, $attribute, $type)
    {
        return ['data-rule-equalTo' => '[name=' . $attribute . '_confirmation]'];
    }
